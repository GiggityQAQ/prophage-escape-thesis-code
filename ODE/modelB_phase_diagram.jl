...
#
# This script simulates Model B under periodic antibiotic
# pressure and generates one phase diagram.
#
# Model B assumption:
...
#       eta_R = eta
#
# Here, A is the antibiotic state:
#   A = 0 during the antibiotic-free phase
#   A = 1 during the antibiotic-present phase
#
...
#   4) DirectPC
#
# Within each single run, the code detects which lysogen
# populations remain present:
#   L_SS, L_SR, L_RS, L_RR
#
# Within-run coexistence is written with commas:
#   L_SR,L_RS
#
# Different outcomes across the four initial-condition tests
# are written with vertical bars:
#   L_SR | L_RS
#
# Example:
#   Sresident  -> L_SR
...
# This will increase the total simulation time substantially.
...
# Maximum number of cycles allowed before stopping a simulation.
max_resident_cycles = 300
max_analysis_cycles = 300

# Presence threshold for the four lysogen populations.
...
output_filename = "modelB_phase_lysogen_presence_by_test_square.png"
...
#
# Model B:
#   ARG-negative prophages use eta_S.
#   ARG-positive prophages use eta_R.
# =========================================================
...
function modelB!(du, u, p::ParamsB, t)
    ...
    # Antibiotic-induced escape applies only to ARG-negative prophages.
...
# communities or the direct-competition community before
...
# long-term state. Rare invaders are then added.
#
# DirectPC starts from a mixed initial condition and does not
# include a resident-establishment phase.
# =========================================================
...
# =========================================================
# Within-run lysogen presence classification
#
# Classification is based on the final analysis window rather
# than a single endpoint. For each lysogen, the maximum density
# in the final window is compared with the extinction threshold.
#
# A single-run label describes which lysogen types are present
# in that trajectory:
#
#   L_SR        : only L_SR is present
#   L_SR,L_RS   : L_SR and L_RS coexist within the same run
#   N           : no lysogen remains above the threshold
# =========================================================
...
function classify_one_run_lysogen_presence(t, X, T_off, T_on;
                                           lysogen_extinction_threshold=1e-10,
                                           final_cycles=5)

    idx = final_window_indices(t, T_off, T_on; n_cycles=final_cycles)

    LSS_max = maximum(X[idx, 3])
    LSR_max = maximum(X[idx, 4])
    LRS_max = maximum(X[idx, 5])
    LRR_max = maximum(X[idx, 6])

    present = String[]

    if LSS_max > lysogen_extinction_threshold
        push!(present, "L_SS")
    end

    if LSR_max > lysogen_extinction_threshold
        push!(present, "L_SR")
    end

    if LRS_max > lysogen_extinction_threshold
        push!(present, "L_RS")
    end

    if LRR_max > lysogen_extinction_threshold
        push!(present, "L_RR")
    end
...
#
# A comma "," means coexistence within one simulation run.
#
# A vertical bar " | " means different outcomes were observed
# across the four initial-condition tests under the same
# antibiotic schedule.
# =========================================================

function within_label_rank(label::String)
    if label == "N"
        return 0
    end
    ...
    # Convert the lysogen-presence label to a binary mask rank.
    rank = 0
...
function combine_four_test_presence_outcomes(outcomes::Vector{String})
    ...
    uniq = sort(uniq, by=within_label_rank)
...
#   1. Build the initial condition for each test.
#   2. Run the post-invasion dynamics.
#   3. Classify lysogen presence in each final trajectory.
#   4. Combine unique outcomes across the four tests.
# =========================================================

function classify_point_modelB_lysogen_by_test(T_on, T_off, p::ParamsB;
                                               rare_density=1e-4,
                                               saveat=1.0,
                                               reltol=1e-7,
                                               abstol=1e-9,
    ...
    for kind in test_kinds
        ...
        t, X, _, _, _ =
            run_to_convergence_with_history(
                ...
                reltol=reltol,
                abstol=abstol,
                max_cycles=max_analysis_cycles,
                ncheck=ncheck,
                tol=convergence_tol
            )

        outcome = classify_one_run_lysogen_presence(
            t,
            X,
            T_off,
            T_on;
    ...
    return combine_four_test_presence_outcomes(outcomes)
end
...
function scan_phase_modelB_lysogen_by_test(p::ParamsB;
                                           T_on_values=10.0:80.0:1200.0,
                                           T_off_values=10.0:80.0:1200.0,
                                           rare_density=1e-4,
                                           saveat=1.0,
    ...
    for i in eachindex(T_off_values)
        ...
        for j in eachindex(T_on_values)
            labels[i, j] = classify_point_modelB_lysogen_by_test(
                T_on_values[j],
                T_off_values[i],
                p;
                rare_density=rare_density,
...
# the number of possible combined labels depends on the scan
# result.
#
# The plotting settings address two common export issues:
#   1. Axis labels can be clipped outside the saved image.
#   2. The heatmap panel can become non-square.
#
# Key settings:
#   - size=(1800, 1800) creates a square canvas.
#   - aspect_ratio=:equal makes one x-unit equal to one y-unit.
#   - xlims and ylims use the same limits to keep the panel square.
#   - margins reserve space for axis labels, title, and legend.
# =========================================================

function plot_phase_modelB_lysogen_by_test(T_on_values, T_off_values, labels;
                                           filename="modelB_phase_lysogen_presence_by_test_square.png")

    present = unique(vec(labels))

    # Sort labels by the number of across-test outcomes and their
    # within-run lysogen-presence ranks.
    function label_sort_key(label::String)
        parts = split(label, " | ")
        ranks = [within_label_rank(String(p)) for p in parts]
        return (length(parts), sum(ranks), label)
    end

    cats = sort(present, by=label_sort_key)
    ...
    # Generate categorical colors automatically from the observed labels.
    ncat = length(cats)
    ...
    plt = heatmap(
        ...
        size=(1800, 1800),
        dpi=300,
        title="Model B Phase Diagram\nLysogen Presence Outcomes by Initial Condition",
        titlefontsize=13,
        guidefontsize=13,
        tickfontsize=10,
        legendfontsize=7,
...
# the plot, and return the numerical label matrix.
# =========================================================
...
function main()
    ...
    T_on_vals, T_off_vals, labels =
        scan_phase_modelB_lysogen_by_test(
            p;
    ...
    plt = plot_phase_modelB_lysogen_by_test(
        T_on_vals,
        T_off_vals,
        labels;
